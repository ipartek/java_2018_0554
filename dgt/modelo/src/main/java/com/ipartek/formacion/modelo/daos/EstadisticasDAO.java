...
public class EstadisticasDAO {
	...
	private static final String SQL_OBJETIVOS_ANYO = "select round(sum(importe),2) as 'objetivo' from multa where id_agente = ? and year(fecha_alta) = ?;";
	private static final String SQL_OBJETIVOS_MES = "select month(fecha_alta) as 'mes', round(sum(importe),2) as 'objetivo' from multa where id_agente = ? and year(fecha_alta) = ? GROUP BY month(fecha_alta);";
	// private static final String SQL_OBJETIVO_ANUAL = "SELECT objetivo FROM
	// objetivo where Anio=?;";
...
//			"Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"));
	...
	public float objetivosAnyo(int idAgente, int anyo) {
		...
		try (Connection conn = ConnectionManager.getConnection();
				PreparedStatement pst = conn.prepareStatement(SQL_OBJETIVOS_ANYO)) {
			pst.setInt(1, anyo);
	...
	public ArrayList<Estadistica> objetivosMes(int idAgente, int anyo) {
		...
		try (Connection conn = ConnectionManager.getConnection();
				PreparedStatement pst = conn.prepareStatement(SQL_OBJETIVOS_MES)) {
			...
			try (ResultSet rs = pst.executeQuery()) {
				while (rs.next()) {
					...
					estadistica.setImporte(rs.getFloat("objetivo"));
