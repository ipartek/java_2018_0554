...
public class VehiculoDAO {
	...
	private static final String SQL_GET_ALL_BY_ID = "SELECT v.id AS 'id_vehiculo', v.numero_bastidor AS 'bastidor', v.matricula AS 'matricula', c.nombre AS 'combustible',  m.nombre AS 'modelo' FROM vehiculo AS v, combustible AS c, modelo AS m WHERE v.id_combustible = c.id AND v.id_modelo = m.id AND v.id_propietario = ?;";

	/**
	 * Obtener todos los vehiculos de una Persona(propetario)
	 * 
	 * @param id_propetario identifacador de la Persona
