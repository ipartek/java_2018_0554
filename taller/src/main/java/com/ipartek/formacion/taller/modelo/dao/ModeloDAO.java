...
public class ModeloDAO implements IDAO<Modelo> {
	...
	private static final String SQL_GET_BY_VEHICULO_ID = "SELECT m.id, m.nombre FROM modelo AS m INNER JOIN  vehiculo AS v ON m.id = v.id_modelo WHERE v.id=?";
	...
	public Modelo getByVehiculoID(int idVehiculo) {
		Modelo m = null;
		try(
	...
	@Override
	public boolean insert(Modelo modelo) throws SQLException { // SI recibe parametro ( UNA ID para identificar el
																// Modelo que queremos CREAR)
	...
	@Override
	public boolean update(Modelo modelo) throws SQLException { // SI recibe parametro ( UNA ID para identificar el
																// Modelo que queremos MODIFICAR)
		boolean resul = false;
		try (Connection conn = ConnectionManager.getConnection();
